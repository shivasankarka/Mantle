...
#  ===----------------------------------------------------------------------=== #
"""Checkpoint (mantle.serialize.checkpoint)
------------------------------------------------
...
def load_checkpoint(path: String, mut parameters: Parameters) raises -> CheckpointInfo:
    ...
    Raises:
        If the file is missing, isn't a mantle checkpoint, or has an
        unsupported version.
    ...
    if magic != CHECKPOINT_MAGIC:
        raise Error("Not a mantle checkpoint file (bad magic): " + path)
