...
struct TensorShape(Stringable):
    ...
    fn __init__[num: Int](out self, shape: IndexList[num]):
        self._rank = num
        self._shape = IndexList[MAX_RANK]()
        for i in range(min(self._rank, MAX_RANK)):
            self._shape[i] = shape[i]

    fn __init__(out self, rank: Int, shape: IndexList[MAX_RANK]):
        self._rank = rank
        self._shape = shape
...
struct Tensor[dtype: DType](Stringable, Movable, Copyable, Movable):
    ...
    fn __init__(
        out self, owned data: UnsafePointer[Scalar[dtype]], owned shape: TensorShape
    ):
    ...
    fn __moveinit__(out self, owned other: Tensor[dtype]):
        self._data = other._data
        self._shape = other._shape

    fn __copyinit__(out self, other: Tensor[dtype]):
